...
ObjTrackRosNode::ObjTrackRosNode()
    : Node("obj_track_ros"), Publisher("obj_track_ros"),
      Subscriber("obj_track_ros") {
  auto cam_descriptor = rcl_interfaces::msg::ParameterDescriptor();
  cam_descriptor.description =
      "List of camera config files to use for tracking";
  cam_descriptor.type =
  ...
  tracked_obj_sub = create_subscription<msg::TrackedObject>(
      "/tracker/objects", 10,
      std::bind(&ObjTrackRosNode::receiveTrackedBody, this, _1));
  tf_broadcaster = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
  tf_buffer = std::make_shared<tf2_ros::Buffer>(this->get_clock());
  tf_listener = std::make_shared<tf2_ros::TransformListener>(*tf_buffer);

  tracker_control_sub = create_subscription<msg::TrackerControl>(
      "/tracker/control", 10,
      std::bind(&ObjTrackRosNode::receiveTrackerControl, this, _1));
  track_result_pub = create_publisher<msg::TrackResult>("/tracker/result", 1);
}

void ObjTrackRosNode::configureCameras(
    const std::vector<std::string> &camera_configs) {
  for (const std::string &filename : camera_configs) {
    ...
    for (std::size_t i = 0; i < root.size(); i++) {
      ...
      if (type == "Ros2ColorCamera") {
        auto camera = std::make_shared<obj_track_ros::Ros2ColorCamera>(
            this, name, image_topic, info_topic, frame, publish_overlay);
        color_cameras.push_back(camera);
        auto renderer = std::make_shared<m3t::FocusedBasicDepthRenderer>(
            name + "_depth_renderer", geometry, camera);
        color_renderers.push_back(renderer);
        if (publish_overlay) {
          auto renderer = std::make_shared<m3t::FullNormalRenderer>(
              name + "_normal_renderer", geometry, camera);
          normal_renderers.push_back(renderer);
        }
      } else if (type == "Ros2DepthCamera") {
        float scale = root[i]["scale"].as<float>(1.0);
        auto camera = std::make_shared<obj_track_ros::Ros2DepthCamera>(
            this, name, image_topic, info_topic, frame, scale);
...
std_msgs::msg::Header ObjTrackRosNode::getLatestImageHeader() {
  ...
  for (auto &camera : color_cameras) {
    ...
    if (current.stamp.sec > latest.stamp.sec ||
        current.stamp.sec == latest.stamp.sec &&
            current.stamp.nanosec > latest.stamp.nanosec) {
      latest = current;
    }
  }
  for (auto &camera : depth_cameras) {
...
void ObjTrackRosNode::receiveTrackedBody(
    const obj_track_ros::msg::TrackedObject::SharedPtr msg) {
...
void ObjTrackRosNode::createBody(
    const obj_track_ros::msg::TrackedObject::SharedPtr msg,
    m3t::Transform3fA link2world_pose) {
  ...
  auto body = std::make_shared<m3t::Body>(
      msg->name, msg->geometry_path, msg->geometry_unit_in_meter,
      msg->geometry_counterclockwise, msg->geometry_enable_culling,
      geometry2body_pose);
  ...
  const std::filesystem::path body_region_model_path{"/tmp/" + msg->name +
                                                     "_region_model.bin"};
  auto region{std::make_shared<m3t::RegionModel>(msg->name + "_region", body,
                                                 body_region_model_path)};

  const std::filesystem::path body_depth_model_path{"/tmp/" + msg->name +
                                                    "_depth_model.bin"};
  auto depth{std::make_shared<m3t::DepthModel>(msg->name + "_depth", body,
                                               body_depth_model_path)};
  ...
  for (int i = 0; i < color_cameras.size(); i++) {
    ...
    auto region_modality{std::make_shared<m3t::RegionModality>(
        msg->name + "_region_modal_" + color_cameras[i]->name(), body,
        color_cameras[i], region)};
  ...
  for (int i = 0; i < depth_cameras.size(); i++) {
    ...
    auto depth_modality{std::make_shared<m3t::DepthModality>(
        msg->name + "_depth_modal_" + depth_cameras[i]->name(), body,
        depth_cameras[i], depth)};
    depth_modality->ModelOcclusions(depth_renderers[i]);
    link->AddModality(depth_modality);
  }

  auto optimizer{
      std::make_shared<m3t::Optimizer>(msg->name + "_optimizer", link)};
  tracker->AddOptimizer(optimizer);

  auto detector{std::make_shared<m3t::StaticDetector>(
      msg->name + "_detector", optimizer, link2world_pose)};
...
bool ObjTrackRosNode::UpdatePublisher(int iteration) {
  ...
  if (normal_renderers.size() > 0) {
    for (int i = 0; i < color_cameras.size(); i++) {
      if (color_cameras[i]->publish_overlay &&
          normal_renderers[renderer_index]->set_up()) {
        auto renderer = normal_renderers[renderer_index];
        renderer->set_camera2world_pose(
            renderer->camera_ptr()->camera2world_pose());
...
void ObjTrackRosNode::receiveTrackerControl(
    const obj_track_ros::msg::TrackerControl::SharedPtr msg) {
  if (msg->is_stopped) {
...
bool ObjTrackRosNode::SetUp() { return true; }

std::shared_ptr<m3t::Tracker> ObjTrackRosNode::getTracker() { return tracker; }
} // namespace obj_track_ros

int main(int argc, char *argv[]) {
  rclcpp::init(argc, argv);
  std::shared_ptr<obj_track_ros::ObjTrackRosNode> node =
      std::make_shared<obj_track_ros::ObjTrackRosNode>();
